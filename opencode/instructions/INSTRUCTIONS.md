...
## Notion Session Logging

You have access to Notion via MCP (server name: `notion`). Log every session to the opencode Session Logs database.
Data source ID: `d4fc1127-a7b8-4828-a961-ad9cf0061c3a`

### Hard rules
- NEVER delete, move, or restructure any Notion content.
- NEVER create pages at workspace root.
- One row per session, append only.
- Tag all content with `(via opencode)`.
- If Notion MCP fails, continue working. Do NOT block on logging.

### Session start — create a DB row with:
- **Session**: Short task + date, e.g. `Fix auth bug — 2026-05-02 (via opencode)`
- **Date**: Today
- **Project**: Repo/directory being worked in
- **Status**: `Partial`

### Track during session (write once at end, not repeatedly):
- Key decisions and why
- Errors or build failures
- Which subagents were invoked

### Session end — update the row with:
- **Status**: `Complete`, `Partial`, or `Blocked`
- **Agents Used**: All subagents invoked
- **Files Changed**: Comma-separated files modified
- **Decisions**: Choices made — approach, architecture, tradeoffs
- **Errors**: Build errors, test failures, blockers
- **Notes**: Anything else relevant

### What counts as a decision worth logging
- Chose X over Y and why
- Deferred something and why
- Found a bug root cause
- Made a breaking or schema change

### What NOT to log
- Routine file reads or lookups
- Intermediate steps with no lasting significance
